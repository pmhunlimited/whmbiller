...
class Security {
    ...
    private function handleFailedAttempt($username, $ip) {
        ...
        $stmt = $this->db->prepare("SELECT id, email FROM users WHERE username = ?");
        ...
        if ($res->num_rows > 0) {
            ...
            // User doesn't exist, block IP automatically as per requirements
    ...
    public function isIPBlocked($ip) {
        ...
        if ($res->num_rows > 0) {
            ...
            if ($row['status'] === 'blacklist') {
                if ($row['block_until'] && strtotime($row['block_until']) > time()) {
                    return true;
                } else if ($row['block_until']) {
                    // Block expired
    ...
    public function checkCountryBlock($ip) {
        ...
        // In a real production environment, you would use a service like MaxMind or a database
